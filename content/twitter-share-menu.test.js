/**
 * Twitter Share Menu Tests
 * Unit tests for the simplified Twitter content script
 */

const { init, getConfig } = require('./twitter-share-menu');

describe('Twitter Share Menu', () => {
  ...
  beforeEach(() => {
    ...
    // Clear twitterIntegration
    global.window.twitterIntegration = null;
  });
  ...
  describe('Unit Tests', () => {
    ...
    test('init() should initialize share menu integration with Twitter config', async () => {
      // Call init
      await init();
      ...
      expect(window.twitterIntegration).toBeDefined();
      expect(typeof window.twitterIntegration.cleanup).toBe('function');
      expect(typeof window.twitterIntegration.isActive).toBe('function');
    });
    ...
    test('init() should handle initialization errors gracefully', async () => {
      ...
      expect(window.twitterIntegration).toBeNull();
    });
    ...
    test('init() should work when ShareMenuIntegration returns minimal controller', async () => {
      ...
      // Verify integration was stored
      expect(window.twitterIntegration).toBeDefined();
      expect(window.twitterIntegration.isActive()).toBe(false);
    });
